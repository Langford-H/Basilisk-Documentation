...
This solver addresses the incompressible Navier-Stokes equations\cite{2003_Popinet,2015_Popinet} on collocated grids i.e. velocity $\mathbf{u}$ as well as pressure $p$ are stored at center of the cell:
...
Once $\mathbf{u}^{***}$ is computed, we obtain $\mathbf{u}^{n+1}$ with:
...
The pressure gradient $\nabla p^{n+1}$ in the previous equation is obtained by applying the incompressibility constraint:

\begin{gather}
    \nabla \cdot \mathbf{u}^{n+1} = 0
\end{gather}

to solve a Poisson equation in the \textbf{Projection} step.
...
\subsection{Advection}\label{sec:centered-advection}
...
To construct the discrete form of the nonlinear advection term $\nabla\cdot(\mathbf{u}\otimes\mathbf{u})^{,n+1/2}$ while preserving conservation, a divergence-free face velocity $\mathbf{u}_f$ is required. Instead of using the classical Rhie–Chow (RC) interpolation \cite{1983_Rhie} commonly employed in finite-volume solvers, \texttt{Basilisk} adopts the BCG scheme, which incorporates both the advective fluxes and the pressure-gradient contributions when computing face velocities. A projection operator is then applied to enforce the divergence-free condition.

Given the face velocity $\mathbf{u}^{,n+1/2}_f$, equation~\eqref{equ:centered-adv} is advanced using functions provided in the header file \texttt{bcg.h}, which implements the full BCG discretization. For additional details on the BCG scheme, please refer to the \texttt{bcg.h} documentation.
...
The final step enforces the non-divergence constraint. A key challenge in this stage is the misalignment introduced by the collocated grid arrangement: velocities are stored at cell centers, whereas pressure gradients are naturally defined on cell faces.
...
\end{gather}
which leads to the following cell-centered pressure equation, obtained by applying the projection operator to the face velocity:
\begin{equation}
    \nabla\!\left(\frac{\nabla p^{n+1}}{\rho^{n+1/2}}\right)
    = \nabla\!\left(\frac{\mathbf{u}_f^{***}}{\Delta t}\right).
\end{equation}

The intermediate face velocity is given by  
\begin{equation}
    \mathbf{u}_f^{***}
    = \frac{\mathbf{u}^{***}[] + \mathbf{u}^{***}[-1]}{2}
    + \Delta t \mathbf{a}_f^{n+1/2}.
\end{equation}

Here we emphasize that, since $\mathbf{u}^{***}$ already includes all contributions except pressure, the face velocity $\mathbf{u}_f^{***}$ can be obtained simply by averaging adjacent cell-centered values and adding the acceleration term. No additional treatment—such as RC interpolation or the BCG construction described in section~\ref{sec:centered-advection}—is required.

Finally, the divergence-free face velocity is updated as  
\begin{equation}
    \mathbf{u}_f^{n+1}
    = \mathbf{u}_f^{***}
    - \frac{\Delta t}{\rho^{n+1/2}}\, \nabla p^{n+1},
\end{equation}
and will be used for tracer advection in the next time step.
