...
\codecomment{
  ...
  The notation of corresponding cell \para{i} is identified according to equation \ref{equ:beg-upwindfinal2}.\\
  \para{s} = $s_d$\\
}
...
\codecomment{
  ...
  \textbf{compute $u_o$ in equation \ref{equ:beg-upwind}}: \\
  \para{vn}(\para{wn})=$(u^{n+1/2}_{f,o}[i,0]+u^{n+1/2}_{f,o}[i,1])/(fm.e[i,0]+fm.e[i,1])$\\
  \textbf{component of other direction}:\\
  The gradient $\overline{\frac{\partial \Phi^n}{\partial x_o}}$ is first computed by upwind scheme based on direction of $u_o$ according to equation \ref{equ:beg-upwind2}\\
