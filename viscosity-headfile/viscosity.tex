...
\begin{multicols}{2}
  \begin{tikzpicture}
    \node [block](init){
        \textbf{Pre-Define}\\
        Define data structure Viscosity called in the total sovler\\
...
\begin{multicols}{2}
  \begin{tikzpicture}
    \node [block](init){
        ...
        implement of Eq.\ref{equ:aim} for different dimension.\\
...
\begin{multicols}{2}
  \begin{tikzpicture}
    \node [block](init){
        ...
        Compute flux \para{tau} on each face for three dimension based on Eq.\ref{equ:2dunfold}.\\
        ...
        Then the residual is updated by Eq.\ref{equ:resi} and stored in \para{res}. The maximum of all components is returned through double data \para{maxres}.
      };
  \end{tikzpicture}
  \columnbreak
  \begin{minted}[obeytabs=false,mathescape=true,breaklines]{lexer.py:DiffLexer -x}
